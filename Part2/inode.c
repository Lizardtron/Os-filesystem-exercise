										 ...
										 struct buffer_head **p)
{
	...
	*p = NULL;
	...
	/* Determine the block group the inode belongs to. */
	...
	/* Compute the block number within the inode table. */
	block = le32_to_cpu(gdp->bg_inode_table) +
	((ino - 1) % inodes_pg) * inode_sz / blocksize;
	...
	/* Set the buffer_head pointer to allow cleanup by the caller. */
	*p = bh;

	/* Return a pointer to the ext2_inode structure within the block. */
...
struct inode *ext2_iget(struct super_block *sb, unsigned long ino)
{
    ...
    ext2_debug("Request to get inode: %lu\n", ino);

    inode = iget_locked(sb, ino);
    if (!inode) {
        ext2_debug("Failed to allocate inode for: %lu\n", ino);
	...
	if(!raw_inode){ext2_debug("Oh no: %lu\n", ino);}
    if (IS_ERR(raw_inode)) {
    ...
    ext2_debug("Filling VFS inode fields for: %lu\n", ino);
	...
	//GIATI XALAS PAIDI MOU TO MOUNT DEN KATALAVAINW
	ext2_debug("8 for: %lu\n", ino);
    inode->i_blocks = le32_to_cpu(raw_inode->i_blocks);
	ext2_debug("9 for: %lu\n", ino);
    inode->i_size = le32_to_cpu(raw_inode->i_size);

	ext2_debug("Done filling VFS inode fields for: %lu\n", ino);
    ...
    ext2_debug("Inode populated successfully: %lu\n", ino);
