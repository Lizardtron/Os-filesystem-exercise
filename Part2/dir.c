                             ...
                             struct folio **foliop)
{
	...
	unsigned reclen = EXT2_DIR_REC_LEN(namelen); // Minimum record length
	...
	for (i = 0; i < npages; i++) {
		/* Get the address of the page (folio) for this directory block */
		kaddr = ext2_get_folio(dir, i, 0, foliop);
		if (IS_ERR(kaddr))
			return ERR_CAST(kaddr);

		/* Start scanning directory entries within the page */
		de = (ext2_dirent *)kaddr;
		char *end = kaddr + ext2_last_byte(dir, i); // End of valid entries in the page

		while ((char *)de < end) {
			/* Check for zero-length directory entry (corruption) */
			if (de->rec_len == 0) {
		...
		/* Release the mapped folio for the current page */
		folio_release_kmap(*foliop, kaddr);
	}
