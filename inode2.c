                                         ...
                                         struct buffer_head **p)
{
    ...
    *p = NULL;
    ...
    /* Determine the block group and get the group descriptor. */
    block_group = (ino - 1) / inodes_pg;
    gdp = ext2_get_group_desc(sb, block_group, NULL);
    if (!gdp)
        goto eio;

    /* Compute offset within the inode table and determine the block. */
...
struct inode *ext2_iget(struct super_block *sb, unsigned long ino)
{
    ...
    /* Allocate the VFS inode structure. */
    ...
    /* Populate the VFS inode structure. */
    ...
    /* Set the inode operations and file operations based on the file type. */
    if (S_ISREG(inode->i_mode)) {
        ...
        if (ext2_inode_is_fast_symlink(inode)) {
            ...
            nd_terminate_link(EXT2_I(inode)->i_data, inode->i_size,
                              sizeof(EXT2_I(inode)->i_data) - 1);
        } else {
    ...
    /* Populate the ext2_inode_info structure. */
    ei = EXT2_I(inode);
    ...
    /* Copy block addresses into the in-memory inode structure. */
