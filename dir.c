							 ...
							 struct folio **foliop)
{
	...
	for (i = 0; i < npages; i++) {
		/* Get the folio for the current page */
		kaddr = ext2_get_folio(dir, i, 0, foliop);
		if (IS_ERR(kaddr))
			return ERR_CAST(kaddr);

		/* Calculate the last byte of the page */
		...
		/* Traverse the directory entries within the page */
		char *limit = kaddr + last_byte;
		for (de = (ext2_dirent *)kaddr; (char *)de < limit; de = ext2_next_entry(de)) {
			if (de->rec_len == 0) {
				folio_release_kmap(*foliop, kaddr);
				return ERR_PTR(-EIO); // Corrupted directory entry
			}

			if (ext2_match(namelen, name, de)) {
				return de; // Found the entry, return it
			}
		}

		/* Release the folio mapping after scanning the page */
		folio_release_kmap(*foliop, kaddr);
	}
